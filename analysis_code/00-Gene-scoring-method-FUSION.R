...
# 获取所有文件列表
all_files <- list.files(file_path, pattern = "maf0.01.txt", full.names = TRUE)

# 获取所有表型名称
phenotypes <- unique(str_extract(basename(all_files), "^[^_]+"))
...
for (phenotype in phenotypes) {
  phenotype_files <- all_files[str_detect(all_files, paste0("", phenotype, "_count_maf0.01.txt"))]

  # 确保文件存在
  if (length(phenotype_files) == 0) {
    warning(paste("No files found for phenotype:", phenotype))
    next
  }

  # 读取并合并所有染色体文件
  merged_data <- lapply(phenotype_files, function(file) {
    if (file.exists(file)) {
  ...
  if (nrow(merged_data) > 0) {
    merged_results[[phenotype]] <- merged_data
  } else {
    warning(paste("Merged data is empty for phenotype:", phenotype))
  }
}
...
if (length(merged_results) > 0) {
  print(dim(merged_results[[1]]))
  head(merged_results[[1]])
}


baso_count <- merged_results$baso
class(baso_count)
...
####------FUsion assessment-------9K

###baso_FUsion
setwd("/Users/mayunlong/Desktop/WMU2024/ctDRTF-codes and data/scHBO/2024-11-20-ctDRTF_re-analysis/02-10-blood_traits/Method_envalue/")

gene_info <- merged_results$baso[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$lymphocyte[,c("ID","TWAS.Z","TWAS.P")]
...
####------lymph_percent_FUsion

setwd("/Users/mayunlong/Desktop/WMU2024/ctDRTF-codes and data/scHBO/2024-11-20-ctDRTF_re-analysis/02-10-blood_traits/Method_envalue/")

gene_info <- merged_results$lymphPercent[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$eosin[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$HL[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$MCHC[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$wbc[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$neutr[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$MCV[,c("ID","TWAS.Z","TWAS.P")]
...
gene_info <- merged_results$monocyte[,c("ID","TWAS.Z","TWAS.P")]
